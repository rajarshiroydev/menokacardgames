...
| Purge job decisions (Step 3L) | Open: scheduler (recommend a daily Vercel Cron call protected by a secret); credential for deleting the Neon Auth identity after 30 days (Neon API key or a narrowly granted SQL role); where failed-purge alerts go; purge ordering (mark `purging`, delete owned rows through the account cascade, then the Auth identity, with audit and retry) |
| Multi-user transition | In progress on `feature/multi-user-transition`; Step 3K account deletion request, lock and 30-day recovery complete; Step 3L automated purge next and required before any release |
